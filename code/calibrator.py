...
def get_calib_data_path():
    img_paths = []
    data_root = "/media/seu/data_store/tiny-imagenet-200/val/"
...
def get_dipoorlet_calib():
    data_root = "/media/seu/data_store/tiny-imagenet-200/val/images/"
    image_list = get_calib_data_path()    
    for i, image_path in enumerate(image_list):
        image = Image.open(data_root + image_path).convert("RGB")
        image = Preprocess(image).numpy()
        image.tofile("/home/seu/workspace/zjh_ws/Dipoorlet/input.1/" + str(i) + ".bin")
